...
if (!$adminToken || !str_starts_with($authHeader, 'Bearer ')) {
...
// Load current settings from DB table api_fetch_settings
$stmt = $pdo->query("SELECT setting_key, setting_value FROM api_fetch_settings");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach ($defaults as $key => $defaultVal) {
        ...
        // Upsert into DB
        $stmt = $pdo->prepare("
            INSERT INTO api_fetch_settings (setting_key, setting_value, updated_at)
            VALUES (:k, :v, NOW())
